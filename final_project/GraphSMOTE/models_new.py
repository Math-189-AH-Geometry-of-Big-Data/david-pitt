...
class SageConv(Module):
    ...
    def forward(self, features, adj):
        ...
        if not isinstance(adj, torch.sparse.FloatTensor):
            # add torch sparse sum
            print(type(adj))
            if len(adj.shape) == 3:
                try:
                    neigh_feature = torch.bmm(adj, features) / (adj.sum(dim=1).reshape((adj.shape[0], adj.shape[1],-1))+1)
                except NotImplementedError():
                    neigh_feature = torch.bmm(adj, features) / (adj.to_dense().sum(dim=1).reshape((adj.shape[0], adj.shape[1],-1))+1)

                #neigh_feature = torch.bmm(adj, features) / (torch.sparse.sum(adj,dim=1).reshape((adj.shape[0], adj.shape[1],-1))+1)
            else:
                #neigh_feature = torch.mm(adj, features) / (adj.sum(dim=1).reshape(adj.shape[0], -1)+1)
                try:
                    neigh_feature = torch.mm(adj, features) / (adj.sum(dim=1).reshape(adj.shape[0], -1)+1)
                except NotImplementedError():
                    neigh_feature = torch.mm(adj, features) / (adj.to_dense().sum(dim=1).reshape(adj.shape[0], -1)+1)
                #neigh_feature = torch.mm(adj, features) / (torch.sparse.sum(adj,dim=1).reshape(adj.shape[0], -1)+1)
        else:
